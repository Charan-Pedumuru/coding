...
class check_crc(gr.basic_block):
    ...
    def __init__(self, include_header=True, verbose=True, force=False):
    ...
    def handle_msg(self, msg_pmt):
        ...
        if not self.force and not header.crc:
            ...
            if len(packet_bytes) < 8:  # 4 bytes CSP header, 4 bytes CRC-32C
                if self.verbose:
                    print('Malformed CSP packet (too short)')
                return
            crc_val = crc(packet_bytes[:-4] if self.include_header else packet_bytes[4:-4])
            packet_crc = struct.unpack('>I', packet_bytes[-4:])[0]
