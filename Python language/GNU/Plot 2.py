...
bit_string = ''.join(map(str, encoded_output))
...
for j, Eb_No in enumerate(Eb_No_dB):
    ...
    noise = noise_std_dev * np.random.randn(len(encoded_output))
    received_signal = encoded_output + noise

    # Decoding
    decoded_bits = viterbi(encoded_output, start_metric, fsm)
