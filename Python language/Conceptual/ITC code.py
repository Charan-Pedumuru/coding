...
print ('Which bit got error during transmission: ' + str(e))
if e > 0:
 x = list(x)
 x[e - 1] = str(1 - int(x[e - 1]))
 x = ''.join(x)
print ('Encoded bit string that got error during transmission: ' + x )
z = ''.join([str(bin(int(j, 2) & int(x, 2)).count('1') % 2) for j in H])
if int(z, 2) > 0:
 e = int(Ht[int(z, 2) - 1], 2)
else:
 e = 0
print ('Which bit found to have error: ' + str(e))
# correct the error
if e > 0:
 x = list(x)
 x[e - 1] = str(1 - int(x[e - 1]))
